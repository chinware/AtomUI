...
void WindowUtils::lockWindowBuddyLayer(ATOMUI_WIN_HANDLE windowHandle, ATOMUI_WIN_HANDLE buddyHandle)
{
    ...
    auto buddy = reinterpret_cast<Window>(buddyHandle);

    Atom type = XInternAtom(display, "_NET_WM_WINDOW_TYPE_UTILITY", False);
    Atom typeList = XInternAtom(display, "_NET_WM_WINDOW_TYPE", False);
    XChangeProperty(display, buddy, typeList, XA_ATOM, 32,
                   PropModeReplace, reinterpret_cast<unsigned char*>(&type), 1);

    XSetTransientForHint(display, buddy, window);

    Atom state = XInternAtom(display, "_NET_WM_STATE_SKIP_TASKBAR", False);
    Atom stateList = XInternAtom(display, "_NET_WM_STATE", False);
    XChangeProperty(display, buddy, stateList, XA_ATOM, 32,
                   PropModeAppend, reinterpret_cast<unsigned char*>(&state), 1);
