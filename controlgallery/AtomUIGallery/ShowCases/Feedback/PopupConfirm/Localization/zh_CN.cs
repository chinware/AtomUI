using AtomUI.Theme.Language;
using AtomUIGallery.Localization;

namespace AtomUIGallery.ShowCases.PopupConfirm;

[LanguageProvider(LanguageCode.zh_CN, PopupConfirmShowCase.LanguageId)]
internal partial class zh_CN : LanguageProvider
{
    public const string BasicUsageTitle = "基础用法";
    public const string BasicUsageDescription = "基础示例支持确认框的标题和描述属性。";
    public const string LocaleTextTitle = "本地化文本";
    public const string LocaleTextDescription = "设置 okText 和 cancelText 属性来自定义按钮标签。";
    public const string PlacementTitle = "弹出位置";
    public const string PlacementDescription = "提供 12 种弹出位置。";
    public const string CustomizeIconTitle = "自定义图标";
    public const string CustomizeIconDescription = "设置 icon 属性来自定义图标。";
    public const string ComponentCategory = "反馈";
    public const string ComponentStatusStable = "稳定";
    public const string PageSubtitle = "在轻量级危险操作或风险操作前进行就地确认。";
    public const string PageDescription = "PopupConfirm 将触发控件与确认浮层组合在一起，适用于需要快速二次确认、但不需要阻塞式模态框的操作。";
    public const string InfoNamespaceLabel = "命名空间";
    public const string InfoPackageLabel = "包";
    public const string InfoBaseClassLabel = "基类";
    public const string ScenarioExamples = "示例";
    public const string ScenarioApi = "API";
    public const string ScenarioDesignToken = "设计变量";
    public const string ApiColumnMember = "成员";
    public const string ApiColumnDescription = "说明";
    public const string ApiColumnType = "类型";
    public const string ApiColumnDefault = "默认值";
    public const string ApiPropertyPopupConfirmTitle = "显示在确认浮层中的标题文本。";
    public const string ApiPropertyPopupConfirmConfirmContent = "显示在确认浮层标题下方的内容。";
    public const string ApiPropertyPopupConfirmConfirmContentTemplate = "用于渲染自定义确认内容的可选模板。";
    public const string ApiPropertyPopupConfirmOkText = "确认按钮显示的文本。";
    public const string ApiPropertyPopupConfirmCancelText = "取消按钮显示的文本。";
    public const string ApiPropertyPopupConfirmOkButtonType = "确认操作使用的按钮类型。";
    public const string ApiPropertyPopupConfirmIsShowCancelButton = "控制是否显示取消按钮。";
    public const string ApiPropertyPopupConfirmIcon = "显示在确认标题前的可选图标。";
    public const string ApiPropertyPopupConfirmConfirmStatus = "确认内容的语义状态。";
    public const string ApiEventPopupConfirmCancelled = "选择取消操作时触发。";
    public const string ApiEventPopupConfirmConfirmed = "选择确认操作时触发。";
    public const string ApiEventPopupConfirmPopupClick = "点击确认浮层任一按钮后触发。";
    public const string TokenColumnToken = "变量";
    public const string TokenColumnDescription = "说明";
    public const string TokenColumnScope = "范围";
    public const string TokenColumnStatus = "状态";
    public const string TokenScopeComponent = "组件";
    public const string TokenStatusStable = "稳定";
    public const string TokenNamePopupMinWidth = "确认浮层面板的最小宽度。";
    public const string TokenNamePopupMinHeight = "确认浮层面板的最小高度。";
    public const string TokenNameButtonSpacing = "确认浮层按钮之间的间距。";
    public const string TokenNameIconMargin = "状态图标外边距。";
    public const string TokenNameContentContainerMargin = "主确认内容区域外边距。";
    public const string TokenNameButtonContainerMargin = "按钮操作区域外边距。";
    public const string TokenNameTitleMargin = "确认标题外边距。";
    public const string P2ConfirmContentAreYouSureToDeleteThisTask = "确定要删除这个任务吗？";
    public const string P2OkTextOk = "确定";
    public const string P2CancelTextCancel = "取消";
    public const string P2TitleDeleteTheTask = "删除任务";
    public const string P2ContentDelete = "删除";
    public const string P2ContentLt = "LT";
    public const string P2ContentLeft = "左侧";
    public const string P2ContentLb = "LB";
    public const string P2ContentTl = "TL";
    public const string P2ContentTop = "顶部";
    public const string P2ContentTr = "TR";
    public const string P2ContentRt = "RT";
    public const string P2ContentRight = "右侧";
    public const string P2ContentRb = "RB";
    public const string P2ContentBl = "BL";
    public const string P2ContentBottom = "底部";
    public const string P2ContentBr = "BR";

    protected override Type GetResourceKindType() => typeof(PopupConfirmShowCaseLangResourceKind);
}
