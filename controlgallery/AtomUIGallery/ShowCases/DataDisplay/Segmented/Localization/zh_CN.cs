using AtomUI.Theme.Language;

namespace AtomUIGallery.ShowCases.Segmented;

[LanguageProvider(LanguageCode.zh_CN, SegmentedShowCase.LanguageId)]
internal partial class zh_CN
{
    public const string BasicTitle = "基础用法";
    public const string BasicDescription = "最基础的用法。";
    public const string BlockSegmentedTitle = "块级分段控制器";
    public const string BlockSegmentedDescription = "block 属性会让 Segmented 适配父容器宽度。";
    public const string DisabledTitle = "禁用";
    public const string DisabledDescription = "禁用状态的 Segmented。";
    public const string ThreeSizesTitle = "Segmented 尺寸";
    public const string ThreeSizesDescription = "Segmented 支持大号（40px）、默认（32px）、小号（24px）和自定义尺寸。";
    public const string IconOnlyTitle = "仅图标";
    public const string IconOnlyDescription = "为 Segmented 项设置图标但不设置标签。";
    public const string WithIconTitle = "带图标";
    public const string WithIconDescription = "为 Segmented 项设置图标。";
    public const string P2ContentDaily = "每日";
    public const string P2ContentWeekly = "每周";
    public const string P2ContentMonthly = "每月";
    public const string P2ContentQuarterly = "每季度";
    public const string P2ContentYearly = "每年";
    public const string P2ContentLongtextLongtextLongtextLongtext = "长文本-长文本-长文本-长文本";
    public const string P2ContentMap = "地图";
    public const string P2ContentTransit = "公交";
    public const string P2ContentSatellite = "卫星";
    public const string P2ContentList = "列表";
    public const string P2ContentAva = "Ava";

    public const string P2ContentKanban = "看板";

    public const string P2ContentAtomUI = "AtomUI";
    public const string ScenarioExamples = "示例";
    public const string ScenarioApi = "API";
    public const string ScenarioDesignToken = "设计令牌";
    public const string PageSubtitle = "将互斥选项组织成紧凑的分段选择器。";
    public const string PageDescription =
        "Segmented 用于展示一组少量相关选项，支持单选、图标、横向铺满和基于 Token 的尺寸样式。";
    public const string ComponentCategory = "数据展示";
    public const string ComponentStatusStable = "稳定";
    public const string ApiColumnProperty = "属性";
    public const string ApiColumnDescription = "说明";
    public const string ApiColumnType = "类型";
    public const string ApiColumnDefault = "默认值";
    public const string ApiPropertySizeType = "控制分段轨道和选项的尺寸。";
    public const string ApiPropertyIsExpanding = "让选项拉伸并填满父容器宽度。";
    public const string ApiPropertyIsMotionEnabled = "启用选中滑块移动和选项背景变化的动画。";
    public const string ApiPropertySelectedIndex = "当前选中项的索引。";
    public const string ApiPropertySelectedItem = "当前选中项或绑定的数据值。";
    public const string ApiEventSelectionChanged = "选中项变化时触发。";
    public const string ApiPropertyIcon = "显示在分段选项文本前的可选图标。";
    public const string ApiPropertyIsSelected = "将某个选项标记为选中状态。";
    public const string TokenColumnToken = "Token";
    public const string TokenColumnDescription = "说明";
    public const string TokenColumnScope = "作用域";
    public const string TokenColumnStatus = "状态";
    public const string TokenNameTrackPadding = "分段轨道周围的内边距。";
    public const string TokenNameTrackBg = "分段轨道背景色。";
    public const string TokenNameItemColor = "分段选项默认文本颜色。";
    public const string TokenNameItemHoverColor = "选项悬浮时的文本颜色。";
    public const string TokenNameItemHoverBg = "选项悬浮时的背景色。";
    public const string TokenNameItemActiveBg = "选项按下时的背景色。";
    public const string TokenNameItemSelectedBg = "选中选项的背景色。";
    public const string TokenNameItemSelectedColor = "选中选项的文本颜色。";
    public const string TokenNameItemMinHeightLG = "大尺寸选项最小高度。";
    public const string TokenNameItemMinHeight = "默认尺寸选项最小高度。";
    public const string TokenNameItemMinHeightSM = "小尺寸选项最小高度。";
    public const string TokenScopeComponent = "组件";
    public const string TokenStatusStable = "稳定";

}
