...
internal class ThemeDefinitionReader
{
   ...
   private bool _inGlobalTokenCtx = false;
   private bool _inControlTokenCtx = false;
   ...
   private void HandleStartElement(XmlReader reader)
   {
      ...
      if (name == ThemeElementName) {
         ...
         EmitErrorMsg(reader, string.Format("Element tag: {0} not supported.", string.Join('.', _currentElementNames)));
      }
   }
   ...
   {
      // 这样处理方便一点
      string algorithmsStr = reader.ReadElementContentAsString();
   ...
   private void HandleStartControlTokenElement(XmlReader reader)
   {
      ...
      string? tokenId = reader.GetAttribute(IdAttrName);
      ...
      string? algorithm = reader.GetAttribute(AlgorithmAttrName);
   ...
   {
      string? tokenName = reader.GetAttribute(NameAttrName);
      if (string.IsNullOrWhiteSpace(tokenName)) {
         EmitRequiredAttrError(reader, NameAttrName);
      }

      string? tokenValue = reader.GetAttribute(ValueAttrName);
