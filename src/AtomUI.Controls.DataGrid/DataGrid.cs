...
public partial class DataGrid : TemplatedControl
{
    ...
    private void HandleRowDetailsVisibilityModeChanged(AvaloniaPropertyChangedEventArgs e)
    {
        ...
        if (newValue != null)
        {
            UpdateRowDetailsVisibilityMode(newValue.Value);
        }
    }
    ...
    public void ScrollIntoView(object? item, DataGridColumn? column)
    {
        ...
        if (item == null && column is not null)
        {
    ...
    {
        if (_verticalScrollChangesIgnored > 0 || _vScrollBar is null)
        {
            return;
        }
    ...
    private bool BeginCellEdit(RoutedEventArgs? editingEventArgs)
    {
        ...
        Debug.Assert(CurrentSlot < SlotCount);
        Debug.Assert(EditingRow == null || EditingRow.Slot == CurrentSlot);
        Debug.Assert(CurrentColumn is not null);
    ...
    private bool CancelRowEdit(bool exitEditingMode)
    {
        ...
        if (dataItem is null || !DataConnection.CancelEdit(dataItem))
        {
            return false;
        }
    ...
    private void FlushCurrentCellChanged()
    {
        ...
        if (_collapsedSlotsTable.Contains(CurrentSlot))
        {
            DataGridRowGroupInfo? rowGroupInfo =
                RowGroupHeadersTable.GetValueAt(RowGroupHeadersTable.GetPreviousIndex(CurrentSlot));
            Debug.Assert(rowGroupInfo != null);
            if (rowGroupInfo != null)
            {
                ExpandRowGroupParentChain(rowGroupInfo.Level, rowGroupInfo.Slot);
    ...
    {
        if (_columnHeadersPresenter != null && dataGridColumn.HeaderCell is not null)
        {
