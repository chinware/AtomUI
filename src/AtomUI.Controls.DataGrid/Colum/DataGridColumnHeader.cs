...
public class DataGridColumnHeader : ContentControl
{
    ...
    private void OnAreSeparatorsVisibleChanged(AvaloniaPropertyChangedEventArgs e)
    {
        ...
        {
            _desiredSeparatorVisibility = e.NewValue != null && (bool)e.NewValue;
            if (OwningGrid != null)
            {
        ...
        else if (mousePositionHeaders.X >= rightEdge)
        {
                ...
                OwningGrid.HorizontalScrollBar.Value < OwningGrid.HorizontalScrollBar.Maximum)
            {
                ...
                scrollAmount = Math.Min(newVal,
                    OwningGrid.HorizontalScrollBar.Maximum - OwningGrid.HorizontalScrollBar.Value);
                OwningGrid.UpdateHorizontalOffset(scrollAmount + OwningGrid.HorizontalScrollBar.Value);
            }
    ...
    {
        var targetColumn =
            GetReorderingTargetColumn(mousePositionHeaders, false /*scroll*/, out double scrollAmount);
        if (targetColumn != null)
        {
            return targetColumn.DisplayIndex > OwningColumn!.DisplayIndex
    ...
    private void OnMouseMove_Reorder(ref bool handled, Point mousePosition, Point mousePositionHeaders)
    {
        ...
        {
            // Find header we're hovering over
            var targetColumn = GetReorderingTargetColumn(mousePositionHeaders, !OwningColumn!.IsFrozen /*scroll*/,
            ...
            if (OwningGrid.ColumnHeaders.DropLocationIndicator != null)
            {
                ...
                if (targetColumn == null || targetColumn == OwningGrid.ColumnsInternal.FillerColumn ||
                    targetColumn.IsFrozen != OwningColumn.IsFrozen)
                {
                    targetColumn =
