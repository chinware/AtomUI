...
public abstract class DataGridBoundColumn : DataGridColumn
{
        ...
        set
        {
            if (_binding != value)
            {
                ...
                if (_binding != null)
                {
                    if (_binding is BindingBase binding)
                    {
                        if (binding.Mode == BindingMode.OneWayToSource)
                        {
                            throw new InvalidOperationException(
                                "DataGridColumn doesn't support BindingMode.OneWayToSource. Use BindingMode.TwoWay instead.");
                        }
                    ...
                    if (OwningGrid != null)
                    {
                        OwningGrid.NotifyColumnBindingChanged(this);
                    }
                }
    ...
    private static ICellEditBinding? BindEditingElement(AvaloniaObject target, AvaloniaProperty property,
                                                        IBinding binding)
    {
