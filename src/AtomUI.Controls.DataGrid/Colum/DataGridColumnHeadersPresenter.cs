...
public sealed class DataGridColumnHeadersPresenter : Panel, IChildIndexProvider
{
    ...
    internal Control? DragIndicator
    {
        ...
        set
        {
            ...
            {
                if (_dragIndicator != null && Children.Contains(_dragIndicator))
                {
    ...
    internal Control? DropLocationIndicator
    {
        ...
        set
        {
            ...
            {
                if (_dropLocationIndicator != null && Children.Contains(_dropLocationIndicator))
                {
    ...
    protected override Size ArrangeOverride(Size finalSize)
    {
        ...
        foreach (DataGridColumn dataGridColumn in OwningGrid.ColumnsInternal.GetVisibleColumns())
        {
            ...
            {
                columnHeader.Arrange(
                    new Rect(scrollingLeftEdge, 0, dataGridColumn.LayoutRoundedWidth, finalSize.Height));
                EnsureColumnHeaderClip(columnHeader, dataGridColumn.ActualWidth, finalSize.Height, frozenLeftEdge,
                    scrollingLeftEdge);
                if (DragColumn == dataGridColumn && DragIndicator != null)
                {
    ...
    protected override Size MeasureOverride(Size availableSize)
    {
        ...
        {
            double adjustment = Double.IsPositiveInfinity(availableSize.Width)
                ? OwningGrid.CellsWidth
                : availableSize.Width - totalDisplayWidth;
