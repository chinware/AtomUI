...
public partial class DataGrid
{
    ...
    internal bool ColumnRequiresRightGridLine(DataGridColumn dataGridColumn, bool includeLastRightGridLineWhenPresent)
    {
        return (GridLinesVisibility == DataGridGridLinesVisibility.Vertical ||
                GridLinesVisibility == DataGridGridLinesVisibility.All) && VerticalGridLinesBrush != null &&
               (dataGridColumn != ColumnsInternal.LastVisibleColumn ||
                (includeLastRightGridLineWhenPresent && ColumnsInternal.FillerColumn.IsActive));
    }
                ...
                for (int i = newDisplayIndex; i < targetColumn.DisplayIndexWithFiller; i++)
                {
                    ...
                    if (trackChange)
                    {
                        column.DisplayIndexHasChanged =
                            true; // NotifyColumnDisplayIndexChanged needs to be raised later on
                    }
                }
            }
            else
    ...
    internal void OnColumnMaxWidthChanged(DataGridColumn column, double oldValue)
    {
        ...
        {
            DataGridLength oldWidth = new(oldValue, column.Width.UnitType, column.Width.DesiredValue,
    ...
    internal void NotifyInsertedColumnPostNotification(DataGridCellCoordinates newCurrentCellCoordinates,
                                                       int newDisplayIndex)
    {
    ...
    private double AdjustStarColumnWidths(int displayIndex, double adjustment, bool userInitiated)
    {
        ...
        var totalStarColumnsWidth      = 0d;
        var totalStarColumnsWidthLimit = 0d;
        var totalStarWeights           = 0d;
        ...
        try
        {
            ...
            for (int displayIndex = 0; displayIndex < ColumnsInternal.DisplayIndexMap.Count; displayIndex++)
            {
                ...
                if (displayIndex >= deletedColumn.DisplayIndexWithFiller)
                {
                    ...
                    column!.DisplayIndexWithFiller = column.DisplayIndexWithFiller - 1;
                    column.DisplayIndexHasChanged = true; // NotifyColumnDisplayIndexChanged needs to be raised later on
                }
            }

            // Now raise all the NotifyColumnDisplayIndexChanged events
            FlushDisplayIndexChanged(true /*raiseEvent*/);
        }
        finally
        {
    ...
    {
        var index                = 0;
        var frozenColumnWidth    = 0d;
        var oldFrozenColumnWidth = 0d;
    ...
    {
        Debug.Assert(dataGridColumn != null);
        Debug.Assert(dataGridColumn.HeaderCell != null);
    ...
    private bool ScrollColumnIntoView(int columnIndex)
    {
            ...
            (columnIndex != DisplayData.FirstDisplayedScrollingCol || _negHorizontalOffset > 0))
        {
                      ...
                      ColumnsInternal.DisplayInOrder(DisplayData.LastTotallyDisplayedScrollingCol, columnIndex)))
            {
                ...
                var newFirstDisplayedScrollingCol =
    ...
    private void GenerateColumnsFromProperties()
    {
        ...
        if (DataConnection!.DataProperties != null && DataConnection.DataProperties.Length > 0)
        {
            ...
            foreach (PropertyInfo propertyInfo in DataConnection.DataProperties)
            {
                ...
                var columnOrder  = DefaultColumnDisplayOrder;
        ...
        else if (DataConnection.DataIsPrimitive && DataConnection.DataType is not null)
        {
            AddGeneratedColumn(
                                                                        ...
                                                                        DataGrid ownerGrid)
    {
        ...
        var newColumn = GetDataGridColumnFromType(propertyType, ownerGrid);
