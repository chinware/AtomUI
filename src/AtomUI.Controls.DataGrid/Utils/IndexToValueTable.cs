...
internal class IndexToValueTable<T> : IEnumerable<Range<T>>
{
    ...
    {
        var rangeIndex = FindRangeIndex(index);
    ...
    public void RemoveValues(int startIndex, int count)
    {
        ...
        if (_list[lowerRangeIndex].LowerBound < startIndex)
        {
            ...
            _list.Insert(lowerRangeIndex,
                new Range<T>(_list[lowerRangeIndex].LowerBound, startIndex - 1, _list[lowerRangeIndex].Value));
            lowerRangeIndex++;
        }
        ...
        else
        {
            ...
            else
            {
                if (lowerRange.Value != null && !lowerRange.Value.Equals(value) && lowerRange.UpperBound >= startIndex)
                {
                    ...
                    {
                        _list.Insert(lowerRangeIndex + 1,
                            new Range<T>(endIndex + 1, lowerRange.UpperBound, lowerRange.Value));
                    }
    ...
    private bool Merge(int lowerRangeIndex)
    {
        ...
        if (lowerRangeIndex >= 0 && upperRangeIndex < _list.Count)
        {
            ...
            Range<T> upperRange = _list[upperRangeIndex];
            if ((lowerRange.UpperBound + 1 >= upperRange.LowerBound) &&
                (lowerRange.Value != null && lowerRange.Value.Equals(upperRange.Value)))
            {
