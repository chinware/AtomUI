...
public class DataGridRow : TemplatedControl
{
    ...
    static DataGridRow()
    {
        ...
        AutomationProperties.IsOffscreenBehaviorProperty
                            .OverrideDefaultValue<DataGridRow>(IsOffscreenBehavior.FromClip);
    }
    ...
    protected override Size ArrangeOverride(Size finalSize)
    {
        ...
        if (!MathUtilities.AreClose(_lastHorizontalOffset, OwningGrid.HorizontalOffset))
        {
        ...
        if (_bottomGridLine != null)
        {
            ...
            gridLineClipGeometry.Rect = new Rect(OwningGrid.HorizontalOffset, 0,
                Math.Max(0, DesiredSize.Width - OwningGrid.HorizontalOffset), _bottomGridLine.DesiredSize.Height);
    ...
    protected override void OnApplyTemplate(TemplateAppliedEventArgs e)
    {
        ...
        if (_detailsElement != null && OwningGrid != null)
        {
            ...
            if (ActualDetailsVisibility && ActualDetailsTemplate != null && _appliedDetailsTemplate == null)
            {
                ...
                SetDetailsVisibilityInternal(ActualDetailsVisibility,
                    raiseNotification: _detailsVisibilityNotificationPending, animate: false);
    ...
    internal void EnsureGridLines()
    {
        if (OwningGrid != null)
        {
            if (_bottomGridLine != null)
            {
                ...
                bool newVisibility = OwningGrid.GridLinesVisibility == DataGridGridLinesVisibility.Horizontal ||
    ...
    private void DetailsContent_HeightChanged(double newValue)
    {
        if (_previousDetailsHeight.HasValue)
        {
            ...
            if (MathUtilities.AreClose(newValue, oldValue) && MathUtilities.AreClose(newValue, _detailsDesiredHeight))
            {
                ...
                for (int ci = 0; ci < nc; ci++)
                {
                    if (columns[ci] is DataGridTemplateColumn column)
                    {
                        column.RefreshCellContent((Control)Cells[column.Index].Content!,
                            nameof(DataGridTemplateColumn.CellTemplate));
                    }
                }
            }
        }
