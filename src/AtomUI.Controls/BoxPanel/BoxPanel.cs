...
public class BoxPanel : Panel
{
    ...
    static BoxPanel()
    {
        ...
        ((IObservable<AvaloniaPropertyChangedEventArgs>)FlexProperty.Changed)
            .AddClassHandler<Control>(
                new Action<Control, AvaloniaPropertyChangedEventArgs>(OnFlexPropertyChanged));
    }
    
    private static void OnFlexPropertyChanged(Control control, AvaloniaPropertyChangedEventArgs e)
    {
        var parentBoxPanel = control.Parent as BoxPanel;
        if (parentBoxPanel != null)
        {
    ...
    protected override Size ArrangeOverride(Size finalSize)
    {
        ...
        foreach (var child in Children)
        {
            ...
            double allocatedSize = (totalFlex > 0)
                ? (Orientation == Orientation.Vertical ? finalSize.Height - originTotalHeight : finalSize.Width - originTotalWidth) *
