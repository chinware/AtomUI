...
public sealed class RangeSelectedDatesCollection : ObservableCollection<DateTime>
{
   /// <summary>
   ...
   /// with this object.
   /// </param>
   public RangeSelectedDatesCollection(RangeCalendar owner)
   {
      _owner = owner;
   ...
   public void AddRange(DateTime start, DateTime end)
   {
      ...
      _owner.OnSelectedDatesCollectionChanged(
         new SelectionChangedEventArgs(SelectingItemsControl.SelectionChangedEvent, _owner.RemovedItems, _addedItems));
   ...
   protected override void InsertItem(int index, DateTime item)
   {
      ...
      if (!Contains(item)) {
         ...
         if (CheckSelectionMode()) {
            if (RangeCalendar.IsValidDateSelection(_owner, item)) {
   ...
   protected override void SetItem(int index, DateTime item)
   {
      ...
      if (!Contains(item)) {
         ...
         if (index >= Count) {
            ...
            if (DateTime.Compare(this[index], item) != 0 && RangeCalendar.IsValidDateSelection(_owner, item)) {
               removedItems.Add(this[index]);
               base.SetItem(index, item);
               addedItems.Add(item);
