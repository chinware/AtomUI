...
public class ContextMenu : AvaloniaContextMenu, IControlCustomStyle
{
   ...
   static ContextMenu()
   {
      PopupFieldInfo = typeof(AvaloniaContextMenu).GetField("_popup", BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.FlattenHierarchy)!;
   ...
   {
      Type parentType = typeof(AvaloniaContextMenu);
   ...
   {
      Type parentType = typeof(ContextMenu);
      if (parentType.TryGetMethodInfo(methodName, out var methodInfo, BindingFlags.NonPublic | BindingFlags.Instance))
      {
