...
public class Segmented : SelectingItemsControl
{
   ...
   static Segmented()
   {
      ...
      AffectsRender<Segmented>(SelectedThumbCornerRadiusProperty, SelectedThumbBgProperty, SelectedThumbBoxShadowsProperty,
   ...
   protected override void OnApplyTemplate(TemplateAppliedEventArgs e)
   {
      ...
      bool hasDefaultSelected = false;
      foreach (var item in Items) {
         if (item is not null) {
            var container = ContainerFromItem(item);
            if (container is not null) {
               if (SelectingItemsControl.GetIsSelected(container)) {
