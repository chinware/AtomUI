...
internal static class PopupUtils
{
    ...
    {
        if (!CanEnabledArrow(placement, anchor, gravity)) return null;
        ...
        IReadOnlyList<ManagedPopupPositionerScreenInfo> screens)
    {
        ...
        if (constraintAdjustment.HasAllFlags(PopupPositionerConstraintAdjustment.ResizeX))
        {
            ...
            if (IsValid(unconstrainedRect)) geo = unconstrainedRect;
        }
    ...
    {
        if (edge.HasAnyFlag(PopupAnchor.HorizontalMask)) edge ^= PopupAnchor.HorizontalMask;

        if (edge.HasAnyFlag(PopupAnchor.VerticalMask)) edge ^= PopupAnchor.VerticalMask;
        return edge;
    }
