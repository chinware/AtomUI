...
/// <summary>
/// CalendarViewCell 对 Automation 暴露 SelectionItem 语义，选中/名称来自同一 Cell Model。
...
internal sealed class CalendarViewCellAutomationPeer : ControlAutomationPeer, ISelectionItemProvider
{
    ...
    protected override string? GetNameCore() => Cell.AutomationName;

    public bool IsSelected => Cell.Model?.IsSelected ?? false;

    public ISelectionProvider? SelectionContainer => Cell.OwnerView is { } owner
        ? ControlAutomationPeer.CreatePeerForElement(owner) as ISelectionProvider
        : null;
