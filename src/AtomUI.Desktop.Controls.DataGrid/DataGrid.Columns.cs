...
public partial class DataGrid
{
    ...
    private double AdjustStarColumnWidths(int displayIndex, double adjustment, bool userInitiated)
    {
        ...
        List<DataGridColumn>? starColumns = null;
        for (int columnDisplayIndex = 0; columnDisplayIndex < displayedColumnCount; columnDisplayIndex++)
        {
            ...
            starColumns ??= new List<DataGridColumn>(displayedColumnCount - columnDisplayIndex);
                                          ...
                                          ColumnWidthTarget targetWidth)
    {
        ...
        int displayedColumnCount = ColumnsInternal.GetDisplayedColumnCount();
        List<KeyValuePair<DataGridColumn, double>>? starColumnPairs = null;
        for (int columnDisplayIndex = 0; columnDisplayIndex < displayedColumnCount; columnDisplayIndex++)
        {
            DataGridColumn column = ColumnsInternal.GetDisplayedColumnAtDisplayIndex(columnDisplayIndex);
            if (!column.Width.IsStar ||
            ...
            double factor = (increase ? Math.Max(0, distanceToTarget) : Math.Min(0, distanceToTarget)) /
                            column.Width.Value;
            starColumnPairs ??=
                new List<KeyValuePair<DataGridColumn, double>>(displayedColumnCount - columnDisplayIndex);
    ...
    private static double DecreaseNonStarColumnWidth(DataGridColumn column, double targetWidth, double amount)
    {
        ...
        if (MathUtils.GreaterThanOrClose(targetWidth, column.Width.DisplayValue))
        {
            return amount;
        }
    ...
    private sealed class AutoGeneratedColumnPropertyAccessor : IPropertyAccessor
    {
        ...
        public object? Value => _target.TryGetTarget(out var target) ? _property.Get(target) : null;

        public bool SetValue(object? value, BindingPriority priority)
        {
            if (!_property.CanSet || !_target.TryGetTarget(out var target))
            {
                return false;
            }
    ...
    private void AddColumnsFromGroupTree(IDataGridColumnGroupItem groupItem)
    {
        if (groupItem is DataGridColumn column)
        {
            ColumnsInternal.Add(column);
        ...
        if (groupItem is DataGridColumnGroupItem gridColumnGroup)
        {
    ...
    private void RemoveColumnsFromGroupTree(IDataGridColumnGroupItem groupItem)
    {
        if (groupItem is DataGridColumn column)
        {
            ColumnsInternal.Remove(column);
