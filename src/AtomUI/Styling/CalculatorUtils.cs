...
internal static class CalculatorUtils
{
   ...
   {
      IList<FontSizeInfo> fontSizePairs = CalculateFontSize(fontSize);
      ...
      double fontSizeMD = fontSizes[1];
      double fontSizeSM = fontSizes[0];
      double fontSizeLG = fontSizes[2];
      double lineHeight = lineHeights[1];
      double lineHeightSM = lineHeights[0];
      double lineHeightLG = lineHeights[2];

      FontMapDesignToken fontMapToken = new FontMapDesignToken
      {
   ...
   public static IList<FontSizeInfo> CalculateFontSize(double baseValue)
   {
      ...
      for (var index = 0; index < 10; ++index) {
         int i = index - 1;
         double baseSize = baseValue * Math.Pow(2.71828, i / 5.0);
         double intSize = index > 1 ? Math.Floor(baseSize) : Math.Ceiling(baseSize);
         // Convert to even
