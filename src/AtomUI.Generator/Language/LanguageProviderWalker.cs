   ...
   public override void VisitClassDeclaration(ClassDeclarationSyntax node)
   {
      ...
      if (classDeclaredSymbol is not null) {
         foreach (var attribute in classDeclaredSymbol.GetAttributes()) {
            if (attribute.ConstructorArguments.Any() && attribute.ConstructorArguments[0].Value is string languageCode) {
               LanguageInfo.LanguageCode = languageCode;
            }
            if (attribute.ConstructorArguments.Any() && attribute.ConstructorArguments[1].Value is string languageId) {
               LanguageInfo.LanguageId = languageId;
            }
            
            if (attribute.ConstructorArguments.Any() && attribute.ConstructorArguments[2].Value is string resourceCatalog) {
