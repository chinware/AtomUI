...
| 内容承载 | 用户数据、展示内容、集合项或操作入口如何进入控件。 | `SourceUri`、`SourceUris`、`CoverSourceUri`、`FallbackSourceUri`、`CoverIndicatorContent`、`LoadingContent`、`ErrorContent`、`ImageScaleStep`。 |
...
- 图片加载结果必须与控件、窗口、弹层或数据 owner 生命周期一致。
