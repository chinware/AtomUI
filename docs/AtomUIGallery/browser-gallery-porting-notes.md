...
- Browser Gallery 不复制 `AboutUsPage` / `PaletteShowCase`，只做一层薄的宿主封装。
...
- Browser 实跑 `AboutUsPage` / `PaletteShowCase` 渲染成功，无 `.splash-error`。
...
- Browser Gallery 当前只接入 `AboutUsPage` 和 `PaletteShowCase`，完整 Workspace / AppView / AppShell 还未实现。
