...
| 控件 | 状态 | 跨包依赖 |
| --- | --- | --- |
| DataGrid | Partial | Primitives.ArrowDecoratedBox, Buttons.Button, TextBlock, Pagination, PopupConfirm, ScrollViewer, Spin, Tooltip.ToolTip, TreeView.TreeViewItemTheme；cell header-state binding / filter flyout lifecycle / filter indicator binding / column/group/row/row group header pointer handler cleanup / special column detach lifecycle / column reorder indicator pen cache / details presenter measure registration / row expander details binding 已完成，详见 [DataGrid](DataGrid/README.md) |
...
| # | 子模块 | 备注 |
| --- | --- | --- |
| T4.1 | DataGrid 核心（DataGrid / DataGridRow / DataGridCell / Presenter 系列） | Partial：DataGridCell header sort / reorder state binding converter 闭包已移除；DataGridRowHeader press 本地 handler 已迁到 class handler，且 owner/template 顺序空引用已修复；DataGridRowGroupHeader press 本地 handler 已迁到 class handler；DetailsPresenter measure registration 已收敛；DataGrid / DataGridRow / 其他 Presenter / virtualization 仍待专项 |
