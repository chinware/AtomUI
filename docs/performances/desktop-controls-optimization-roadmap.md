...
| # | 子模块 | 备注 |
| --- | --- | --- |
| T4.1 | DataGrid 核心（DataGrid / DataGridRow / DataGridCell / Presenter 系列） | Partial：DataGridCell header sort / reorder state binding converter 闭包已移除，cell clip repeated arrange 分配已收敛；DataGridRowHeader press 本地 handler 已迁到 class handler，且 owner/template 顺序空引用已修复；DataGridRowGroupHeader press 本地 handler 已迁到 class handler；DataGrid core input handlers 已迁到 virtual override；DataGridRowsPresenter scroll gesture handler 已迁到 class handler，presenter clip geometry repeated arrange 分配已收敛；DataGridRow bottom gridline clip repeated arrange 分配已收敛；DataGridDetailsPresenter clip repeated arrange 分配已收敛；DetailsPresenter measure registration 已收敛；DataGrid / DataGridRow / 其他 Presenter / virtualization 仍待专项 |
