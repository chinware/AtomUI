...
| 已完成基线 | 47 项 | `docs/performances/README.md` "当前文档" / "总列表" 的 Done 行 |
...
| Splitter | **Pending** | 0 | 1 | 完整鼠标拖拽链路 |
| Statistic | **Pending** | 0 | 1 | 计数动画热路径 |
| Steps | **Pending** | 1 | 1 | Tour 复用 |
...
| TimePicker | **Pending** | 1 | 4 | DatePicker 复用 |
...
| Upload | 10 | Pending | Buttons, Empty, Form, ImagePreviewer, Result, Select, Tag, TextBlock |
...
Phase C (Tier 1 复合) ────────────────────────────────────────
  T1.1 TreeView       前置 done → T1.3 TreeSelect / T1.2 Transfer
  T2.1 Calendar       并行
  T1.4 TimePicker     T2.1 done 后
  T1.5 Upload         T0.4 Tag done 后
  T2.4 Steps          → T1.6 Tour
  T1.7 TabControl     独立

Phase D (Tier 2 单功能) ──────────────────────────────────────
  T2.2 Statistic / T2.3 Timeline / T2.5 Breadcrumb / T2.6 Splitter
