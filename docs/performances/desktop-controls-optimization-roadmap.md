...
| 控件 | 状态 | 跨包依赖 |
| --- | --- | --- |
| DataGrid | Partial | Primitives.ArrowDecoratedBox, Buttons.Button, TextBlock, Pagination, PopupConfirm, ScrollViewer, Spin, Tooltip.ToolTip, TreeView.TreeViewItemTheme；cell header-state binding / filter flyout lifecycle / filter indicator binding / column/group/row/row group header pointer handler cleanup / core input handler cleanup / rows presenter scroll gesture cleanup / special column detach lifecycle / column reorder indicator pen cache / details presenter measure registration / row expander details binding 已完成，详见 [DataGrid](DataGrid/README.md) |
