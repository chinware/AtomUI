...
| T4.2 | 列模型（DataGridColumn / DataGridColumnHeader / FilterFlyout / SortIndicator） | Partial：filter flyout closed-state 内容已延迟；filter indicator visibility binding 已收敛；column header hover / press / release / move 本地 handler 已清理；column group header press / release forwarding handler 已清理；其余 sort / resize / reorder 仍待专项 |
