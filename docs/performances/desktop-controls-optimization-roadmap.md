...
| 控件 | 状态 | 跨包依赖 |
| --- | --- | --- |
| DataGrid | Partial | Primitives.ArrowDecoratedBox, Buttons.Button, TextBlock, Pagination, PopupConfirm, ScrollViewer, Spin, Tooltip.ToolTip, TreeView.TreeViewItemTheme；filter flyout lifecycle / filter indicator binding / column header hover handler cleanup / special column detach lifecycle / column reorder indicator pen cache 已完成，详见 [DataGrid](DataGrid/README.md) |
