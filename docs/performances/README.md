...
| Data Display | DataGrid | Partial | [DataGrid](DataGrid/README.md)；filter flyout 内容延迟到首次打开，关闭态过滤菜单项 `7 -> 0`；filter indicator 可见性由 `MultiBinding` 收敛到 `TemplateBinding`，本轮 `DataGridShowCase` repeated mean 提升约 `6.27%` |
