...
| Primitives / InfoPickerInput + AddOnDecoratedBox | 本轮已完成正确性与生命周期优化 | [Primitives](Primitives/README.md) | `PickerClearUpButton` 本地 Click handlers `1 -> 0`，`AddOnDecoratedBox` ContentFrame pointer handlers `4 -> 0` / instance；隐藏 clear direct-click side effects `1 -> 0`；DatePicker/TimePicker/AddOn 状态验证通过，页面 timing 不作为本轮收益证明 |
...
| Shared Primitive | Primitives / InfoPickerInput | Done (correctness + lifecycle) | [Primitives](Primitives/README.md)；`PickerClearUpButton` 本地 Click handler `1 -> 0`，`AddOnDecoratedBox` ContentFrame pointer handlers `4 -> 0` / instance，隐藏 clear direct-click side effects `1 -> 0` |
