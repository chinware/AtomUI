...
| 控件/体系 | 状态 | 目录 | 关键结果 |
| --- | --- | --- | --- |
| AddOnDecoratedBox | 本轮已完成 | [Primitives](Primitives/README.md) | 默认 LineEdit visual/root `26 -> 20`，KB/item `503.8 -> 398.4`，默认 icon scan `360 -> 0`；ContentFrame pointer handlers `4 -> 0` / instance；Select/TreeSelect/Cascader right-addon template 绑定已修复 |
...
| Grid | 本轮已完成正确性修复与结构优化 | [Grid](Grid/README.md) | 修复 `Row` cached child layout 在 `Col.Span/Order` 与 `IsVisible` 变化后的 stale 问题；same-width measure→arrange `BuildLayout` 调用 `2 -> 1`，页面速度不声明 |
...
| Primitives / InfoPickerInput + AddOnDecoratedBox + CandidateList | 本轮已完成正确性、生命周期与交互结构优化 | [Primitives](Primitives/README.md) | `PickerClearUpButton` 本地 Click handlers `1 -> 0`，`AddOnDecoratedBox` ContentFrame pointer handlers `4 -> 0` / instance；`CandidateList.Items20` 键盘候选移动 container checks `20 -> <=2`；页面 timing 不作为本轮收益证明 |
...
| Splitter | 本轮已完成 Gallery 实测 + runtime structural | [Splitter](Splitter/README.md) | hidden collapse icon 不再构造/保留 `PathIcon`，lazy preview transform 复用；动态 children add/remove temp objects `3 -> 1`，panel refresh temp objects `2 -> 0`，resize event sizes temp objects `3 -> 1`；`SplitterShowCase` repeated mean `36.91ms -> 34.66ms`，P95 `51.79ms -> 40.43ms`，alloc `5770.31KB -> 5659.40KB` |
...
| Transfer | 本轮已完成 Gallery 实测 + structural | [Transfer](Transfer/README.md) | `TargetKeys` membership lookup 改为每次刷新复用 `HashSet`，空 target panel 直接短路；`TreeTransfer` target 递归结果列表从 `N -> 1`；`TransferShowCase` repeated mean `242.72ms -> 222.12ms`，cold mean `855.88ms -> 768.42ms`，visuals/logical 不变 |
| TreeView | 本轮已完成 Gallery 实测 + render/interaction structural | [TreeView](TreeView/README.md) | `NodeSwitcherButton` 模板 5 个图标 presenter 收敛到 1 个；`TreeViewShowCase` visuals `2108 -> 1646`、`IconPresenter 332 -> 112`、repeated mean `332.44ms -> 213.41ms`；连接线 repeated render `Pen` allocations `1-2 -> 0 after first`，拖拽 indicator render frame `1 -> 0 after first`；checkbox parent aggregation child scans `2 -> 1` |
...
| TabControl | 本轮已完成 Gallery 实测 + render structural | [TabControl](TabControl/README.md) | 默认无 icon/非 closable tab 不再创建隐藏 slot；`TabControlShowCase` visuals `1583 -> 1405`，repeated mean `170.75ms -> 157.35ms`；tab strip border repeated render `Pen` allocations `1/control -> 0 after first` |
...
| Shared Primitive | AddOnDecoratedBox | Done | [Primitives](Primitives/README.md)；输入类控件共享视觉与 addon 体系；ContentFrame pointer handlers `4 -> 0` / instance；Select/TreeSelect/Cascader right-addon template 绑定已修复 |
...
| Shared Primitive | Primitives / InfoPickerInput / CandidateList | Done (correctness + lifecycle + interaction structural) | [Primitives](Primitives/README.md)；`PickerClearUpButton` 本地 Click handler `1 -> 0`，`AddOnDecoratedBox` ContentFrame pointer handlers `4 -> 0` / instance，`CandidateList.Items20` 键盘候选移动 container checks `20 -> <=2` |
...
| Layout | Grid | Done (correctness + structural) | [Grid](Grid/README.md)；修复 `Row` stale layout cache，新增 6 个控件级场景和 3 个状态校验；timing smoke-only |
...
| Layout | Splitter | Done | [Splitter](Splitter/README.md)；hidden collapse icon 不再构造/保留 `PathIcon`，lazy preview transform 复用，动态 children add/remove temp objects `3 -> 1`，panel refresh temp objects `2 -> 0`，resize event sizes temp objects `3 -> 1`，`SplitterShowCase` repeated mean 提升约 `6.10%`，P95 提升约 `21.93%` |
...
| Navigation | TabControl | Done (render structural) | [TabControl](TabControl/README.md)；默认 tab item hidden slot 按需化，`TabControlShowCase` repeated mean 提升约 `7.85%`，visuals `1583 -> 1405`；tab strip border repeated render `Pen` allocations `1/control -> 0 after first` |
...
| Data Entry | Transfer | Done | [Transfer](Transfer/README.md)；`TargetKeys` lookup 复用 `HashSet`，空 target panel 直接短路，`TreeTransfer` target 递归结果列表 `N -> 1`，`TransferShowCase` repeated mean 提升约 `8.49%`，cold mean 提升约 `10.22%` |
...
| Data Display | Tag | Done | [Tag](Tag/README.md)；非 closable Tag 减少约 `2KB/instance`，Gallery 76 实例约减少 `140KB` 加载分配 |
...
| Data Display | TreeView | Done | [TreeView](TreeView/README.md)；`NodeSwitcherButton` 单 presenter 化，真实 `TreeViewShowCase` repeated mean 提升约 `35.80%`，visuals `2108 -> 1646`；连接线 repeated render `Pen` allocations `1-2 -> 0 after first`，拖拽 indicator render frame `1 -> 0 after first`，checkbox parent aggregation child scans `2 -> 1` |
