...
验证补充：本轮复跑 `--verify-treeview-states` 仍失败在既有 `TreeViewItemHeader` highlighted-match foreground-only 断言；本轮只改 `TreeView.Filter` 的闭包 `HashSet` 容量，没有触碰 `TreeViewItemHeader.BuildFilterHighlightRuns()` 或 verifier 断言，因此不把该失败归因于本轮容量收敛。
