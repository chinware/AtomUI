...
内部关闭/互斥子菜单路径原先通过 `INavMenuElement.SubItems` 属性取子项；该属性为了公共接口兼容仍返回 `LogicalChildren.OfType<INavMenuItem>()`。本轮把内部热路径改为直接遍历 `LogicalChildren.Count/indexer` 并做同样的 `is INavMenuItem` 类型过滤，公共接口不变。
