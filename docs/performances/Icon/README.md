...
本目录记录 `Icon` 体系的性能分析、优化计划、基线与阶段结果。当前阶段已完成 Phase 2 render 热路径收敛。
...
Phase 2 后，真实 `IconShowCase` repeated navigation 从 `193.31ms` 降到 `151.15ms`，提升 `21.8%`；分配从 `57900.83KB` 降到 `47020.96KB`，下降 `18.8%`。运行时 visual/logical 形态不变，节点减重仍待后续阶段处理。
