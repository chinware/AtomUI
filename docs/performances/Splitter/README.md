...
| `src/AtomUI.Desktop.Controls/Splitter/SplitterPanel.cs` | lazy preview 复用同一个 `TranslateTransform` 并只更新 `X/Y` |
| `tools/performances/AtomUI.Performance/Suites/Splitter/SplitterStateVerification.cs` | 状态验证按 Theme Static Rule 调整为“模板按钮保留，但隐藏态不持有 icon”；新增 transform reuse 验证 |
