...
> 本文档规定 AtomUI 控件的 Theme 文件夹布局、ControlTheme AXAML 结构、以及 Theme 注册方式。
...
| 模板部件使用 `PART_` 前缀命名 | 如 `Name="PART_WaveSpirit"` |
...
模板部件的 `Name` 属性 **必须** 使用 `PART_` 前缀的字符串：
...
## 4. Theme 注册（`ControlNameThemes.axaml`）
...
| ❌ 模板部件 `Name` 不使用 `PART_` 前缀 | 必须遵循 Avalonia 模板部件命名约定 |
