...
## 2. `[DynamicDependency]` 注解的作用

### 2.1 .NET Trimming 问题

.NET 发布模式支持 [IL Trimming](https://learn.microsoft.com/en-us/dotnet/core/deploying/trimming/trim-self-contained)，在发布时会分析代码依赖关系，移除未被静态引用的类型成员。由于反射访问是 **运行时动态发现** 的，Trimmer 无法通过静态分析得知这些成员被使用，因此可能将其裁剪掉，导致运行时 `MissingMethodException` 或 `MissingFieldException`。

### 2.2 `[DynamicDependency]` 的保护机制

`System.Diagnostics.CodeAnalysis.DynamicDependencyAttribute` 是 .NET 提供的 Trimmer 保护注解。当标注在某个成员上时，它告诉 Trimmer：**"即使目标成员看起来未被使用，也必须保留"**。

```csharp
// 告诉 Trimmer：保留 Visual 类型的所有非公开方法
...
### 2.3 `DynamicallyAccessedMemberTypes` 常用值

| 值 | 保护目标 |
|---|---|
| `NonPublicMethods` | 所有非公开方法（private、internal、protected） |
| `NonPublicProperties` | 所有非公开属性 |
| `NonPublicFields` | 所有非公开字段 |
| `NonPublicEvents` | 所有非公开事件 |
