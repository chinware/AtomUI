...
- 相同来源共享编码数据获取，相同来源和解码尺寸共享解码；不同尺寸不得错误共享 Bitmap。
...
- 网络内容在解码前执行协议、按平台可观测能力的重定向、长度、MIME、Magic Bytes、尺寸、像素数和格式校验；Browser
  不虚构不可观测的中间跳审计。
- 远程 SVG 始终拒绝；受信任 `avares` SVG 通过显式注册的本地 vector codec 进入统一 `Source` API。
...
| `AtomUI.Controls.Shared` | `IImageLoader`、`ImageLoader`、`ImageLoaderStore`、来源模型、请求调度、在途合并、缓存、HTTP、本地读取、安全校验、raster codec | ControlTheme、Avatar fallback、Previewer 当前项策略 |
| `AtomUI.Controls` | `AsyncImage`、`IImageLoadControl`、`ImageLoadController`、Avatar 来源与 fallback 状态、本地 SVG codec 注册 | Previewer 导航、窗口和 overlay |
...
| decoded key | encoded key 再加入解码尺寸、格式和 codec 版本后的图片键 |
...
| trusted vector | 由应用资源显式提供、通过 AtomUI 注册 codec 解码的本地 SVG；不包括 HTTP 响应 |
...
| Previewer loader 同时识别 Bitmap 和 SVG | raster 由 Shared codec 处理；受信任 Asset SVG 由 Controls 显式 codec 处理；远程 SVG 拒绝 |
