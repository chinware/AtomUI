...
- `Splash`：视觉控件，承载品牌、文本、进度、状态、扩展内容和静态 API 委托入口。
- `SplashWindow`：桌面窗口宿主，承载一个 `Splash` 实例，管理展示、最短展示时间、关闭延迟和淡出。
- `SplashController`：状态写入入口，统一把 message、detail、progress、status 写入 `Splash`。
- `ISplashService`：实例服务契约，适合应用启动代码、测试和依赖注入场景。
- `SplashService`：默认服务实现，一次管理一个 `CurrentWindow` 和一个 `CurrentController`。
...
- 视觉状态 owner 是 `Splash`，窗口生命周期 owner 是 `SplashWindow`，编排状态 owner 是 `SplashService`。
- `Splash.DefaultService` 是静态 API 的唯一默认服务入口，静态方法不得绕过它操作窗口。
- `SplashController` 不持有业务异常、主窗口或应用 lifetime。
...
| --- | --- | --- | --- | --- | --- | --- |
| `Splash` | public control | `Splash.cs` / `SplashTheme.axaml` | 用户或 `SplashWindow` | 视觉 API、静态 API | public | 可作为用户 XAML 控件和静态入口。 |
| `SplashWindow` | public window | `SplashWindow.cs` / `SplashWindowTheme.axaml` | 用户或 `SplashService` | 窗口展示和关闭节奏 | public | 可直接使用，但不接管主窗口。 |
| `SplashController` | public helper | `SplashController.cs` | `SplashWindow` | 状态更新方法 | public | 只更新 Splash 状态，不保存业务对象。 |
...
| `PART_RootLayout` | template part | `SplashTheme.axaml` | `Splash` template | 背景、边框、圆角、阴影 | template-stable | 主题维护可依赖名称，用户不直接操作。 |
...
- `SplashService.CloseAsync()` 必须在成功、取消和异常路径释放 `CurrentWindow`、`CurrentController` 和订阅。
...
- `SplashController` 对 `Progress`、`IsIndeterminate`、`Status` 的优先级归一。
...
- 控件 API 或行为变更运行 `tests/AtomUI.Desktop.Controls.Extras.Tests`。
