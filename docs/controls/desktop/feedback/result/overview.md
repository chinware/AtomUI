...
本文档定义 `Result` 桌面版的最新设计定位、公共契约、状态模型、视觉主题关系和兼容边界。通用控件研发约束见 [控件研发标准](../../../../engineering/control-development-guidelines.md)，内部实现原理见 [Result 桌面版实现原理](implementation.md)，Result Token 的专项设计见 [Result Token 设计](token.md)，设计和契约变化记录见 [Result Changelog](changelog.md)。
...
- `ResultIndicator`：控件核心或内部协作类型，维护 public surface 与主题可观察行为。
- `ResultToken`：组件 Token scope，负责从全局 token 派生控件语义变量。
...
Result 的视觉选项通过 public API 归一为 theme variables、伪类或模板绑定。Token 保存组件语义值，不能保存实例运行时状态或业务色值。
...
| LLMS 内容 | 来源 | 说明 |
| --- | --- | --- |
| 单控件完整文档 | `overview.md` + Gallery API / Token / ShowCase | 生成 `controls/result/index-cn.md` |
| 单控件语义文档 | `overview.md` + `implementation.md` + theme/template 信息 | 生成 `controls/result/semantic-cn.md` |
