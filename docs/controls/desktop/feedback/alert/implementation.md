...
本文档描述 Alert 桌面版的内部实现范围、源码职责、状态流、生命周期、资源边界和维护规则。公共设计与 API 契约见 [Alert 桌面版架构设计](overview.md)，变化记录见 [Alert Changelog](changelog.md)。涉及组件 Token 的实现应同时阅读 [Alert Token 设计](token.md)。
...
- 内容与数据：`CloseIcon`、`Description`、`IsShowIcon`。
- 交互与状态：`IsClosable`、`IsMessageMarqueeEnabled`。
- 其他稳定入口：`ExtraAction`、`Message`、`Type`。
