...
本文档描述 Icon 桌面版的内部实现范围、源码职责、状态流、生命周期、资源边界和维护规则。公共设计与 API 契约见 [Icon 桌面版架构设计](overview.md)，变化记录见 [Icon Changelog](changelog.md)。涉及组件 Token 的实现应同时阅读 [Icon Token 设计](token.md)。

## 1. 实现定位

本文档覆盖 Icon 的控件实现、主题接入、状态同步和 Gallery 可见维护边界。具体属性注册、默认值、绘制细节和 AXAML selector 仍应直接阅读源码；本文只记录维护者必须理解的稳定结构和不变量。

## 2. 源码文件结构

主要源码文件：

- `src/AtomUI.Controls/Icon/IconFuncTemplate.cs`
- `src/AtomUI.Controls/Icon/IconPresenter.cs`
- `src/AtomUI.Controls/Icon/IconTemplate.cs`
