...
本文档定义 `WindowTitleBar` 桌面版的最新设计定位、公共契约、状态模型、视觉主题关系和兼容边界。通用控件研发约束见 [控件研发标准](../../../../engineering/control-development-guidelines.md)，内部实现原理见 [WindowTitleBar 桌面版实现原理](implementation.md)，WindowTitleBar Token 的专项设计见 [WindowTitleBar Token 设计](token.md)，设计和契约变化记录见 [WindowTitleBar Changelog](changelog.md)。
...
| Gallery 页面 | 未独立 Gallery 页面；以 `docs/controls/desktop/window/window-title-bar` 源文档为准 |
| 控件状态 | Stable |

WindowTitleBar 是 AtomUI 桌面控件体系中的窗口标题栏控件，用于承载标题、图标、拖拽区域和窗口系统按钮。

WindowTitleBar 不负责完整 Window、导航栏或普通工具栏。这些职责应由业务层、组合控件或更专用的 AtomUI 控件承担。
...
WindowTitleBar 的设计语言围绕控件职责、可观察状态和主题契约组织，而不是围绕模板节点组织。

| 维度 | 含义 | WindowTitleBar 中的表达 |
| --- | --- | --- |
| 产品语义 | 控件在界面中承担的稳定职责。 | WindowTitleBar 是 AtomUI 桌面控件体系中的窗口标题栏控件，用于承载标题、图标、拖拽区域和窗口系统按钮。 |
| 内容承载 | 用户数据、展示内容、集合项或操作入口如何进入控件。 | `CheckedIcon`、`IconHeight`、`IconWidth`、`LeftAddOnTemplate`、`LogoTemplate`、`NormalIcon`、`RightAddOnTemplate`、`Title` 等 9 项。 |
...
WindowTitleBar 的公共契约由 public/protected 类型成员、Avalonia 属性、事件、命令、template part、伪类、ControlTheme key 和资源 key 共同组成。维护时应先确认这些契约是否已经被源码、Gallery 示例或文档暴露。
...
- 类型：`CaptionButton`、`CaptionButtonGroup`、`WindowTitleBar`、`WindowsCaptionButton`。
- 枚举：`WindowTitleBarLogoVisibility`。
...
WindowTitleBar 与同分类控件共享尺寸、状态、Token、Gallery 展示和验证规则。组合或派生控件应显式说明哪些 API 被继承、覆盖或不支持。
...
WindowTitleBar 的当前项状态必须由单一 owner 推导。public 选择属性、集合项容器和伪类之间只能做单向同步，集合替换、清空和模板重套用时必须回放当前状态。

### 8.2 弹层与宿主模型

WindowTitleBar 涉及弹层、窗口或 overlay 宿主时，打开状态、取消事件、定位和宿主释放必须保持一致。重复打开、关闭、窗口失活和 template reapply 都必须释放旧宿主引用。

### 8.3 动效模型

WindowTitleBar 的动效只表达状态变化反馈，不应改变 public API 语义。初始加载、禁用态和卸载路径应能抑制或取消动效，避免保留旧控件实例。

## 9. 文档导航、LLMS 导出与验证策略

关联文档：

- [WindowTitleBar 桌面版实现原理](implementation.md)
...
| LLMS 内容 | 来源 | 说明 |
| --- | --- | --- |
| 单控件完整文档 | `overview.md` + Gallery API / Token / ShowCase | 生成 `controls/window-title-bar/index-cn.md` |
| 单控件语义文档 | `overview.md` + `implementation.md` + theme/template 信息 | 生成 `controls/window-title-bar/semantic-cn.md` |
...
| 状态模型 | 覆盖 selection/checked/active、open/close、motion、disabled、hover、pressed、focus 以及控件特有状态。 |
