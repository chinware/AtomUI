...
- Behavior
  - Restore the floating-handle hover shift for the suffix group: since the suffix group moved into the spinner content segment (outside the decorated box `ContentRightAddOn` slot), the animated `ButtonSpinnerDecoratedBox.ContentRightShift` no longer reached it through the template. `NumericUpDown` now reacts to the decorated box hover state and animates `PART_SuffixGroup` through its own `DoubleTransition` (mirroring the decorated box easing/duration). Re-mirroring the animated property frame by frame was rejected: the per-frame relay fights the render clock and shows up as a one-frame jump ("flicker") when the pointer re-enters quickly.
