...
- Behavior
  - Restore the floating-handle hover shift for the suffix group: since the suffix group moved into the spinner content segment (outside the decorated box `ContentRightAddOn` slot), the animated `ButtonSpinnerDecoratedBox.ContentRightShift` no longer reached it through the template. `NumericUpDown` now mirrors that animated property onto `PART_SuffixGroup` through a `TranslateTransform` wired in `SetupTemplatePartBindings`, so hovering the floatable-mode control slides the suffix away from the floating handle again.
