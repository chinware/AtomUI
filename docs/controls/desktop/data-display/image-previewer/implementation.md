...
- 图片源模型文件只表达 `ImageSourceUri`、`ImagePreviewItem`、`LoadedImageSource`、加载调度器和加载服务契约，不承载视觉模板逻辑。
...
- 图片来源：`SourceUri`、`SourceUris`、`FallbackSourceUri`。
...
- `CurrentIndexProperty` 变更、`SourceUri` / `SourceUris` 变更、fallback 状态变化和 effective items 重建都必须触发当前预览项重新解析。
- `CoverIndexProperty` 变更、`SourceUri` / `SourceUris` 变更、fallback 状态变化和 effective items 重建都必须触发封面 item 重新解析。
...
`ImagePreviewItemState` 只能由 item owner 写入。加载任务返回时必须校验 generation、来源身份和取消状态；过期结果必须释放后丢弃，不能覆盖新来源。`SourceUri` / `SourceUris` 的 fallback 决策必须由当前来源集合加载批次统一完成，单个 `ImagePreviewItem` 失败只能更新自身状态，不能直接替换整组 `EffectiveItems`。
...
| --- | --- | --- | --- | --- | --- | --- |
| `ImagePreviewer` / `ImageGroupPreviewer` | public control | `ImagePreviewerTheme.axaml`、`ImageGroupPreviewerTheme.axaml` | 控件实例 | `SourceUri`、`SourceUris`、`CurrentIndex`、`CoverIndex`、`IsOpen` | public | 可作为用户 API 和主题入口理解。 |
...
| source load batch | runtime algorithm | `AbstractImagePreviewer` loading path | `AbstractImagePreviewer` 当前加载轮次 | `SourceUri`、`SourceUris`、`FallbackSourceUri`、`EffectiveItems` | internal-observable | 是集合加载结果归并 owner；负责判断成功项保留、失败项跳过、全部失败 fallback 和旧批次防回写。 |
...
- 图片加载任务由 `ImagePreviewLoadScheduler` 创建和持有取消令牌。`SourceUri` 变更、`SourceUris` 替换、弹层关闭、控件 detach 和 owner dispose 时必须取消仍在运行且不再需要的加载。
- `SourceUri` / `SourceUris` 每次物化都必须形成当前来源集合加载批次。批次完成前，单项失败不得触发 `FallbackSourceUri` 替换整组结果；批次完成后若存在成功项，effective items 保留成功项并跳过失败项；批次全部失败时才进入 fallback。
...
- `PART_ErrorPresenter`：承载图片加载失败内容；当前加载批次全部失败且存在 `FallbackSourceUri` 时优先展示 fallback 结果。没有可用 fallback 时显示 `ErrorContent` / `ErrorContentTemplate` 或默认本地化失败占位。
...
- `SourceUri` / `SourceUris` 加载批次归并：单项失败只更新 item 状态；批次至少一个成功时保留成功项并跳过失败项；批次全部失败且存在 `FallbackSourceUri` 时才替换为 fallback；旧批次完成时必须通过取消状态和当前 effective items 身份校验避免回写新状态。
...
- 大图集合不得 eager load。`SourceUris` 包含 100 张图片时，关闭态只加载封面；打开态只加载当前项、封面项和预加载窗口内图片。
...
- `SourceUri`、`SourceUris` 和 `FallbackSourceUri` 的加载、取消、来源身份和失败处理一致性。
