...
`ImagePreviewItemState` 只能由 item owner 写入。加载任务返回时必须校验版本号和取消状态；过期结果必须释放后丢弃，不能覆盖新来源。`SourceUri` / `SourceUris` 的 fallback 决策必须由当前来源集合加载批次统一完成，单个 `ImagePreviewItem` 失败只能更新自身状态，不能直接替换整组 `EffectiveItems`。
