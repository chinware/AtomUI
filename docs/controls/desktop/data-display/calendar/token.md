...
本文档定义 Calendar 组件 Token 的专属语义、分类与兼容边界。控件 Token 的通用规则见 [AtomUI 控件 Token 设计规范](../../../../engineering/control-token-guidelines.md)。Calendar 整体架构见 [Calendar 桌面版架构设计](overview.md)，实现原理见 [Calendar 桌面版实现原理](implementation.md)，变更记录见 [Calendar Changelog](changelog.md)。

## 1. 定位

新 Calendar 的 Token 收敛为 Ant Design 6 Calendar 的六个公开视觉语义。日期值高度、周标题高度、内容高度、Padding、Border、Typography 与 Motion 均从 SharedToken 派生，不扩大 Calendar 专属 Token surface。
...
| 主题映射 | 走查 today、selected、outside、disabled、focused 状态视觉。 |
