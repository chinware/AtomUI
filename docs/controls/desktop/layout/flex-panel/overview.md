...
本文档定义 `FlexPanel` 桌面版的最新设计定位、公共契约、状态模型、视觉主题关系和兼容边界。通用控件研发约束见 [控件研发标准](../../../../engineering/control-development-guidelines.md)，内部实现原理见 [FlexPanel 桌面版实现原理](implementation.md)。FlexPanel 没有独立 Token 文档；主题主要复用 SharedToken 或关联控件 Token。设计和契约变化记录见 [FlexPanel Changelog](changelog.md)。
...
FlexPanel 与同分类控件共享尺寸、状态、Token、Gallery 展示和验证规则。组合或派生控件应显式说明哪些 API 被继承、覆盖或不支持。

主要协作类型：

- `FlexPanel`：控件主类型，维护 public API、状态同步和主题协作。
...
| LLMS 内容 | 来源 | 说明 |
| --- | --- | --- |
| 单控件完整文档 | `overview.md` + Gallery API / Token / ShowCase | 生成 `controls/flex-panel/index-cn.md` |
...
| 状态模型 | 覆盖 collection/filter、disabled、hover、pressed、focus 以及控件特有状态。 |
