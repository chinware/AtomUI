...
**目标：** 为 Menu、NavMenu 和 DataGrid 建立 Semantic Part 契约，不得造成高密度实例化、虚拟化、Popup 或容器生命周期行为退化。
...
**控件文档：** `docs/controls/desktop/navigation/menu/overview.md`、`docs/controls/desktop/navigation/menu/implementation.md`；只有已批准的 public Part 模型改变稳定的 popup-scroll 契约时，才更新现有 `popup-scroll-design.md`。

**证据范围：** `src/AtomUI.Desktop.Controls/Menu/**/*.cs`、全部 `src/AtomUI.Desktop.Controls/Menu/Themes/*Theme.axaml`；测试 `tests/AtomUI.Desktop.Controls.Tests/Menu`；共享 Gallery `controlgallery/AtomUIGallery/ShowCases/Navigation/Menu`；性能检查 `tools/performances/AtomUI.Performance/Suites/Menu`。

**风险类型：** 分层 item 容器、submenu Popup root、ContextMenu、scroll host、pointer/keyboard 热路径。

- [ ] **记录改造前基线：** 运行：

```bash
dotnet run --project tools/performances/AtomUI.Performance/AtomUI.Performance.csproj --framework net10.0 -- --suite menu --count 60 --markdown /tmp/atomui-semantic-menu-before.md
dotnet run --project tools/performances/AtomUI.Performance/AtomUI.Performance.csproj --framework net10.0 -- --verify-menu-states
```

记录 total/ms per item、KB/item、Visual/root、Logical/root 和状态验证结果；改造后必须使用相同场景进行比较。

- [ ] **Gate A 设计审核：** 审计 `Menu`、`ContextMenu`、`MenuItem`、`MenuSeparator` 和 `MenuPopupScrollHost` 的 owner。确认 frame/items、item toggle/icon/label/gesture/submenu indicator 及 popup frame/items 是否属于稳定职责；嵌套 CheckBox/RadioButton 继续作为独立 owner。记录 top-level 与 submenu 模板、overlay/PopupRoot、滚动、item 生成、open/close 和容器 clear/reuse。
- [ ] 更新两份控件文档，写明准确的 Descriptor、每个已实例化 owner/container 的 marker 数量、cross-root/runtime 标志、排除的 wrapper、生命周期、性能/AOT 和验证矩阵；运行 LLMS verify 和 `git diff --check`；随后停止并等待用户批准。
- [ ] **Gate B 实现：** 新增聚焦测试 `tests/AtomUI.Desktop.Controls.Tests/Menu/MenuSemanticPartTests.cs`，覆盖 Menu/ContextMenu/item/separator owner、嵌套 owner 隔离、层级 add/remove/reset、submenu open-close-reopen、scroll host 模式、keyboard/pointer 状态，并证明不存在动态 marker 变更。
- [ ] 在现有共享 Menu ShowCase 中添加延迟创建的 Menu Semantic Parts 界面；选择 Tab 前不得实例化 Menu 或 NavMenu 的 Semantic 示例。
- [ ] **记录改造后基线：** 重新运行与改造前完全相同的两条命令，输出到 `/tmp/atomui-semantic-menu-after.md`。解释 ms/item、KB/item、Visual/root 或 Logical/root 的每项实质变化；静态 class-marker 元数据不得新增 Visual/Logical 节点。
- [ ] 运行 Generator Semantic 测试、全部 Menu 测试、GalleryBase 测试、`MenuShowCasePageTests`、LLMS verify、NativeAOT publish 和 `git diff --check`。
- [ ] **强制停止：** 保持 Menu 改动未提交，直到用户审核文档、行为和改造前后性能证据，并明确授权提交。
...
- [ ] 只有 Menu 和 NavMenu 的 Gallery 设计都获得批准后，才能将 NavMenu 集成到共享 Menu ShowCase 的 Semantic Tab；不得向真实应用 sidebar 附加 Preview 行为。
...
- [ ] 确认 Menu、NavMenu 和 DataGrid 分别拥有用户授权的独立提交。
- [ ] 重新运行全部六条性能/状态命令，将最终对比归档到全量改造收尾报告，不写入正式控件文档。
