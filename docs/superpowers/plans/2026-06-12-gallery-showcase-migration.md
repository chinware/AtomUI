...
| P3.4 | Implemented | Form | `controlgallery/AtomUIGallery/ShowCases/DataEntry/Form` | `FormShowCase.axaml`; `FormApiDataGrid.axaml`; `FormDesignTokenDataGrid.axaml`; old Basic/Controls/Dynamic/Layout/Preset/State/Validation sub-showcase files merged into root and removed | Structure + snapshot + lazy tabs passed; Gallery tests passed; Gallery Desktop build passed; desktop controls tests passed; `git diff --check` passed. |
