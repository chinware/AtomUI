...
| P2.4 | Implemented | Expander | `controlgallery/AtomUIGallery/ShowCases/DataDisplay/Expander` | `ExpanderShowCase.axaml`; `ExpanderApiDataGrid.axaml`; `ExpanderDesignTokenDataGrid.axaml`; `ExpanderBasicShowCase.axaml`; `ExpanderAppearanceShowCase.axaml`; `ExpanderBehaviorShowCase.axaml` | Structure + snapshot + lazy tabs passed; `AtomUIGallery.Tests` passed; Gallery Desktop build passed; `git diff --check` passed. |
