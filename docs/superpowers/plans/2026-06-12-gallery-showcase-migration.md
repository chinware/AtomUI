...
| P3.10 | Implemented | Space | `controlgallery/AtomUIGallery/ShowCases/Layout/Space` | `SpaceShowCase.axaml`; `SpaceApiDataGrid.axaml`; `SpaceDesignTokenDataGrid.axaml`; old Basic/Align/CompactButton/CompactForm/Size sub-showcase files merged into root and removed | Structure + snapshot + lazy tabs passed; Gallery tests passed; Gallery Desktop build passed; `git diff --check` passed; waiting for user acceptance. |
