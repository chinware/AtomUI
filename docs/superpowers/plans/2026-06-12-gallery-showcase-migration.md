...
| P3.6 | Implemented | Transfer | `controlgallery/AtomUIGallery/ShowCases/DataEntry/Transfer` | `TransferShowCase.axaml`; `TransferApiDataGrid.axaml`; `TransferDesignTokenDataGrid.axaml`; old Basic/Advanced/TreeStatus sub-showcase files merged into root and removed | Structure + snapshot + lazy tabs passed; Gallery tests passed; Gallery Desktop build passed; desktop controls tests passed; `git diff --check` passed. |
