...
| P2.5 | Implemented | List | `controlgallery/AtomUIGallery/ShowCases/DataDisplay/List` | `ListShowCase.axaml`; `ListApiDataGrid.axaml`; `ListDesignTokenDataGrid.axaml`; `ListBasicShowCase.axaml`; `ListAdvancedShowCase.axaml` | Structure + snapshot + lazy tabs passed; `AtomUIGallery.Tests` passed; Gallery Desktop build passed; `git diff --check` passed. |
