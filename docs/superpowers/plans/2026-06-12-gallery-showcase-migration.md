...
| P3.9 | Implemented | Grid | `controlgallery/AtomUIGallery/ShowCases/Layout/Grid` | `GridShowCase.axaml`; `GridApiDataGrid.axaml`; `GridDesignTokenDataGrid.axaml`; old Basic/Spacing/Alignment/Order/ColInfo sub-showcase files merged into root and removed | Structure + snapshot + lazy tabs passed; Gallery tests passed; Gallery Desktop build passed; `git diff --check` passed. |
