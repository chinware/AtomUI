...
| P2.7 | Implemented | AutoComplete | `controlgallery/AtomUIGallery/ShowCases/DataEntry/AutoComplete` | `AutoCompleteShowCase.axaml`; `AutoCompleteApiDataGrid.axaml`; `AutoCompleteDesignTokenDataGrid.axaml` | Structure + snapshot + lazy tabs passed; `AtomUIGallery.Tests` passed; Gallery Desktop build passed; `AtomUI.Desktop.Controls.Tests` passed; `git diff --check` passed. |
