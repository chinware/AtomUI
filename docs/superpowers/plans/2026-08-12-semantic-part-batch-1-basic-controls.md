...
- [ ] 更新三份控件文档：`overview.md` 保留支持摘要，`semantic-part.md` 写明准确的 Part/selector/ContractType/cardinality/customization/cross-root/runtime 公共契约、逐 Part 说明和排除边界，`implementation.md` 维护真实节点映射与生命周期；运行 LLMS verify 和 `git diff --check`；随后停止并等待用户批准。
