...
- `LostFocus` 事件: `RoutedEventArgs` → `FocusChangedEventArgs`（两个事件现在都使用 `FocusChangedEventArgs`）
- 参考 Avalonia 的 `InputElement` 中的事件定义
...
3. 检查所有焦点事件处理器，确保使用正确的事件参数类型（`FocusChangedEventArgs`）
4. 测试 Popup 的打开/关闭行为，确保动画和交互正常工作

---

## SelectableTextBlock 移植问题与解决方案

### 问题 8: TextFormatting 命名空间和文本源类
**症状**: `AtomUI.Media.TextFormatting` 命名空间不存在，`InlinesTextSource` 和 `FormattedTextSource` 类型无法找到

**解决方案**:
- 移除错误的 `using AtomUI.Media.TextFormatting;` 导入
- 在 SelectableTextBlock.cs 中定义 `FormattedTextSource` 和 `InlinesTextSource` 结构体
- 这两个类型在 Avalonia 12 中是内部实现，需要在 AtomUI 中重新实现
- 示例:
  ```csharp
  // 移除
  ...
  // 在 SelectableTextBlock 类后定义
  internal readonly struct FormattedTextSource : ITextSource { ... }
  internal new readonly struct InlinesTextSource : ITextSource { ... }
  ```

**关键点**:
- `GenericTextRunProperties` 构造函数签名改变：参数顺序为 `typeface, fontRenderingEmSize, textDecorations, foregroundBrush, ...`
- `GenericTextParagraphProperties` 不再有 `SetLineSpacing()` 方法，改为在构造函数中传递 `lineHeight` 参数
- `SetTextAsync()` 是 `IClipboard` 的扩展方法，需要导入 `Avalonia.Input.Platform` 命名空间
- `Math.Clamp()` 替代 `MathUtilities.Clamp()`（.NET 6+ 内置方法）

### 问题 9: GenericTextRunProperties 构造函数参数顺序
**症状**: 编译错误 "Argument X: cannot convert from..."

**解决方案**:
- 旧的参数顺序（错误）: `typeface, FontFeatures, FontSize, TextDecorations, Foreground`
- 新的参数顺序（正确）: `typeface, FontSize, TextDecorations, Foreground, fontFeatures: FontFeatures`
- 示例:
  ```csharp
  // 旧代码（错误）
...
## ScrollViewer 移植问题与解决方案

### 问题 7: TopLevelUtils 命名空间导入
**症状**: `TopLevelUtils` 类型无法找到

**解决方案**:
- 添加 `using AtomUI.Controls.Utils;` 命名空间导入
- TopLevelUtils.GetDesktopScaling() 在 AtomUI.Controls.Shared 中存在，在 Avalonia 12 中仍然适用
- 示例:
  ```csharp
  // 添加 using
...
**关键点**:
- TopLevelUtils 是 AtomUI 自定义的工具类，在 Avalonia 12 中仍然可用
- 它内部实现了 `TopLevel.GetTopLevel(control) as WindowBase` 的逻辑
- 只需确保正确导入命名空间即可
